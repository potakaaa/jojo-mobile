...
test('renders Toggle on and off without throwing', async () => {
  await render(
    <Toggle mode="light" value onValueChange={() => {}} label="Marketing notifications" />,
  );
  await render(
    <Toggle mode="light" value={false} onValueChange={() => {}} label="Marketing notifications" />,
  );
}, 15000);
...
test('fires onValueChange when the switch is toggled', async () => {
  ...
  const { getByRole } = await render(
    <Toggle mode="light" value={false} onValueChange={onValueChange} />,
  );
