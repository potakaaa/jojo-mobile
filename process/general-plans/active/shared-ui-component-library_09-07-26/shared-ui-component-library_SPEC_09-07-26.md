...
**Existing component pattern to replicate** (`packages/ui/src/jojo-button.tsx`): named export, `interface {Name}Props`, variant union type mapped through `Record<Variant,string>` lookup tables, built on RN `Pressable`, `style` passthrough prop, `StyleSheet.create` at module bottom, tokens imported directly from `./theme`. No `useTheme()` hook exists in the package — `BrandWordmark` instead takes a `mode: ThemeMode = 'light'` prop and indexes `Colors[mode]` directly (the package has no theme-context dependency on the app).
