...
4. **Plain `fetch` with `Cookie` header for mobile API calls** — `authClient.$fetch` resolves relative paths against better-auth's `/api/auth` base URL, causing 404 on `/api/staff/*` routes. Instead, staff API calls use plain `fetch` with `Cookie: authClient.getCookie()` to forward the SecureStore session cookie (see `apps/mobile/src/features/staff/lib/staff-api.ts` lines 6–21 for the documented rationale).
