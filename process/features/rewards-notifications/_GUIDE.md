...
accrual/unlock/redemption and push notification delivery (order status, promotions). No notifications
...
- `packages/api/src/db/schema/coupons.ts` — partial unique index `coupons_user_reward_unique` on `(user_id, reward_id) WHERE reward_id IS NOT NULL` (migration 0006).
- `packages/api/src/db/schema/star_transactions.ts` — partial unique index on `(order_id, type) WHERE order_id IS NOT NULL` (migration 0005).
- `packages/api/drizzle/0005_nosy_genesis.sql` — star_transactions idempotency index.
- `packages/api/drizzle/0006_windy_dexter_bennett.sql` — coupons reward-unlock idempotency index.
...
| `completed/star-003-reward-unlock_14-07-26/` | VERIFIED (STAR-003) | Battle-pass unlock + coupon gen inside STAR-001 credit tx, migration 0006, coupon-code gen, notification helper, 4-tier roadmap seed, 99 API tests (10 new); `DbCoupon`/`CouponStatus` types |
...
|---|---|---|
| STAFF-003 | Staff order-complete/refund endpoints calling `creditStarForCompletedOrder`/`reverseStarForRefundedOrder` | STAFF-002 (active orders screen) or direct endpoint work |
...
| Coupon redemption at checkout (STAR-004) | Spend `available` coupons at checkout | STAR-003 delivered |
