...
- **Permission request**: a permission-request seam is triggered by a concrete, scoped moment —
  right after a customer places their first order (see Open Questions #5) — not on first app
  launch. Declining does not block or break any other part of the app; the user can still see
  everything in the in-app Notifications list either way. This pass builds the on-device request
  flow and its outcome handling as a **local stub** (no real OS permission dialog appears this
  pass, and no push token is registered anywhere — both are #75's job; see PLAN Assumption A2).
...
9. **Declining permission in the local stub seam does not crash the app or block any other
   flow** (checkout, order tracking, browsing all continue to work normally). No real OS dialog
   exists this pass — the decline path is simulated via the seam's `__DEV__` mock-result trigger.
   proven by: Agent-Probe manual walkthrough (using the `__DEV__` trigger to simulate a decline;
   no RN runner exists for automated on-device verification).
   strategy: Agent-Probe

10. **The permission seam is triggered after the customer's first order is placed, not on first
    app launch**, and is not re-shown again within the same app session once answered
    (session-scoped — the flag is in-memory and resets on reload; cross-launch persistence is
    #75's job).
    proven by: Agent-Probe manual walkthrough (fire-once timing verified via the `__DEV__`
    trigger; OS-level permission timing cannot be verified from a pure-TS unit test).
    strategy: Agent-Probe
...
12. **Every mock notification item carries an explicit `type` and `targetScreen`** matching one of
    the **9** documented notification kinds (4 transactional + 5 marketing, see Background) —
    no notification is generated with an unmapped or missing target. Two pairs of these 9 types
    share a `targetScreen` (`one_more_order` + `reward_unlocked` → rewards; `new_deal` +
    `branch_promo` → deal details), so the 9 types resolve to only 4 distinct target screens —
    the overlap is in target screens, not in the type count.
...
- The notification type model must be able to express all **9** distinct kinds referenced across
...
- `packages/types/src/notifications.ts` is currently stale versus the real DB `notifications`
  ...
  `type` to the **9** real kinds) since `target_screen` is exactly what powers tap-to-navigate and the
...
- **Existing state verified this session:**
  ...
  - PRD §6.12 and §14 confirm the exact wording used for each of the 9 notification kinds and the
